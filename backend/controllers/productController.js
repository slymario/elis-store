...
const addProduct = asyncHandler(async (req, res) => {
    try {
        const {name, description, category, price, quantity, brand, image} = req.fields;
        ...
        switch (true) {
            ...
            case !description:
                return res.json({error: "Description must be provided" });
            case !category:
                return res.json({error: "Category must be provided" });
            case !price:
                return res.json({error: "Price must be provided" });
            case !quantity:
                return res.json({error: "Quantity must be provided" });
            case !brand:
...
const fetchAllProducts = asyncHandler(async (req, res) => {
    try {
        ...
        console.error(error);
        res.status(404).json({error: "All Products not found"});
    }
});
